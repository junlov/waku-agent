...
2. The learner's diff: `git diff learner/chapter-NN-start` (their whole
   attempt, excluding later harness maintenance). If that tag is missing,
   stop and ask them to establish the clean baseline before writing a fix.
